...
\subsection{Benchmark Selection and Execution}
...
of 4 threads for each SPEC benchmark to run on the testbed 
built with gcc 4.4 and g95 without hyperthreading.
...
For the case of benchmark mixes (described in Subsection~\ref{sec:cross-funct-unit} later), 16 threads are generated for 
each benchmark mix for execution on those physical cores,
since each benchmark mix contains four SPEC benchmarks.
On the other hand, every PARSEC benchmark yields more threads
for execution, since ......
...
The benchmark run times (which reflect performance degradation) increase
negligibly always, ranging from 2.1\% to 2.9\%.

% Involving both thermal-aware scheduling and load balancing,
% TAS compares favorably with earlier workload migration-oriented methods,
% such as Core Hopping~\cite{Choi2007}, Variation-Aware Core Hopping
% \cite% {Kursun2008}, and Predict-and-Act~\cite{Ayoub2009}. 
% with comparable reductions in
% average core die temperatures and performance impact for similar
% workloads and processors.  For instance, 
% Specifically, Core Hopping \cite{Choi2007} and Variation-Aware Core Hopping
% \cite{Kursun2008} were demonstrated to lower core temperatures ranging from
% 2.0$^{\circ}$C to 5.5$^{\circ}$C under SPEC CPU2006 benchmarks 
% executed an IBM POWER5 processor.  
% Meanwhile, Predict-and-Act (which employs a hybrid reactive/proactive 
% migration policy) was shown to reduce on-die core temperatures
% by 6$^{\circ}$C to 8$^{\circ}$C on a "simulated"
% 6-core Alpha 21264 processor (rather than a real implementation)
% when executing mixes of SPEC2006 benchmarks similar to those in our study.
