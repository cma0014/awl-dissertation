...
% bm[10] -> powerhat
    ...
    overallLen = length(overall.ProcPwr) / nsize;
    overallTS = 1:overallLen;
    overallPwr = overall.ProcPwr(1:overallLen);
    ...
    h=sqrt(hy*hx);
    overallKSR = ksrlin(overallTS,overallPwr,h,overallLen);
    ...
    for i = 1:size(benchmarks)
        ...
        r = ksrlin(pts, pow, h, powlen);
        powerhat = transpose(r.f);
        [benchmarks{i,3} benchmarks{i,4} benchmarks{i,5} benchmarks{i,6} benchmarks{i,7} ...
         benchmarks{i,8} benchmarks{i,11}] =  testcbm(bmname,pow,powerhat);
        benchmarks{i,9} = pow;
        benchmarks{i,10} = powerhat;
        benchmarks{i,12} = power;
        dumppred(bmname,pow,powerhat);
        hurst = hurst_estimate(pow,'absval',0,1);
    ...
    for i = 1:size(testmarks)
        ...
        pow = power(1:powlen);
        hx=median(abs(pts-median(pts)))/0.6745*(4/3/powlen)^0.2;
        hy=median(abs(pow-median(pow)))/0.6745*(4/3/powlen)^0.2;
        h=sqrt(hy*hx);
        r = ksrlin(pts,pow,h,powlen);
        powerhat = transpose(r.f);
        [testmarks{i,3} testmarks{i,4} testmarks{i,5} testmarks{i,6} testmarks{i,7} ...
         testmarks{i,8} testmarks{i,11}] =  testcbm(bmname,pow,powerhat);
        testmarks{i,9} = pow;
        testmarks{i,10} = powerhat;
        testmarks{i,12} = power;
        dumppred(bmname,pow,powerhat);
    end
    prettyprt(testmarks);
    prterror(testmarks);
