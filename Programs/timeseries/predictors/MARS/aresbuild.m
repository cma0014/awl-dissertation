...
% Please give a reference to the software webpage in any publication
% describing research performed using the software, e.g. like this:
% Jekabsons G. ARESLab: Adaptive Regression Splines for Matlab, 2009,
...
if (~isfloat(Xtr)) || (~isfloat(Ytr))
    error('Data has not a floating-point type.');
end
...
if size(Ytr,2) ~= 1
    error('The vector Ytr should have one column.');
end
...
for j = start : length(model.knotdims)
    if length(model.knotdims{j}) < trainParams.maxInteractions
        ...
        if trainParams.useMinSpan ~= 1
            ...
            for di = alloweddims %for each dimension
                % add new unique basis functions to the list (all except
                % those which for which the parent basis function is zero
                % on the knot site)
...
%s = floor(3 - log2(0.05/d)); % explicit version
...
if nz == 0
    ...
    if param < 0 % automatic
        %s = floor(-log2(-log(1-0.05)/(d*nz)) / 2.5); % explicit version
