function [benchmarks testmarks overall] = nprediction3(varargin)
fprintf('\n\n Flugelnogelfritzend\n\n');
...
procPwrTotal = zeros(50,1);
dataTotal = zeros(50,depvarsize);
counts = zeros(50,1);
...
for i = 1:numBenchmarks
    ...
    if (smooth == 0) 
        [ProcPwr{i} data{i}] = nbuildmodel3(bmname);
    else
        [ProcPwr{i} data{i}] = nbuildmodel3(bmname,smooth,dofigures);
    end        
    if (dofigures == 1)
        h = figure;
        subplot(2,1,1);
...
fprintf(['\n\nPHASE 3: CHECK MODEL AGAINST BENCHMARKS USED TO CREATE MODEL ' ...
    ...
    for i = 1:size(benchmarks)
        ...
        bmcolor = benchmarks{i,2};
        if (smooth == 0) 
            [power data] = nbuildsp3(bmname);
        else
            [power data] = nbuildsp3(bmname,smooth,dofigures);
        end;
